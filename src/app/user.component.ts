...
@Component({
  ...
  <div class="form-group row" [style.display]='isHelperChecked? "block":"none"'>
    ...
    <div class="col-xs-4">
      <input  id="latitude-input" class="form-control"  [style.display]='isHelperChecked? "block":"none"'  type="text" placeholder="latitude" #newLatitude value='{{helperLatitude}}'/>
    </div>
    <label for="longitude-input" class="col-xs-2 col-form-label"> Longitude</label>
    <div class="col-xs-4">
      <input  id="longitude-input" class="form-control"  [style.display]='isHelperChecked? "block":"none"'  type="text" placeholder="longitude" #newLongitude value='{{helperLongutite}}'/>
    </div>
  </div>
  ...
  <button (click)="addUser(newName.value, newPhone.value, newLatitude.value, newLongitude.value, newAddress.value,newHomeAddress.value, newWorkAddress.value )">Add
  </button>
  </div>
</div>
  `
